...
const ENTITY_COLORS = ['#F36B21', '#334155', '#16A34A', '#D88A12', '#DC2626', '#8B5CF6'];
const situacionColor = (sit: number) => (sit === 1 ? '#16A34A' : sit <= 3 ? '#D88A12' : '#DC2626');
...
export default function Page2({ data }: { data: InformeLocativoData }) {
  ...
  actualRows.forEach((row) => { byBankMap[row.entidad] = (byBankMap[row.entidad] || 0) + row.monto; });

  const bankSlices: DonutSlice[] = Object.entries(byBankMap).sort(([, a], [, b]) => b - a).map(([label, value], index) => ({
    label,
    value,
  ...
  const barData: BarDataPoint[] = data.periodosHistoricosAgregados.slice(-24).map((row) => ({ label: row.periodo, value: row.montoTotal }));
  ...
  const lineSeries: LineSeries[] = entidades.map((entidad, index) => ({
    label: entidad,
    color: ENTITY_COLORS[index % ENTITY_COLORS.length],
    data: Object.fromEntries(historicoRows.filter((row) => row.entidad === entidad).map((row) => [row.periodo, row.monto])),
  }));

  const entidadesHistoricas = entidades.map((entidad) => {
    ...
    <section className="page">
      <header className="page-continue-header">
        <div className="continue-brand">
          ...
          <div><div className="continue-title">InmoRent</div><div className="continue-sub">Análisis histórico BCRA</div></div>
        </div>
        <div className="continue-id">{data.documento}{data.denominacion ? ` · ${data.denominacion}` : ''}</div>
      </header>

      <div className="section-title">Distribución actual</div>
      <div className="section-note">Composición del último período informado por BCRA.</div>
      <div className="chart-grid-two">
        <div className="chart-card"><h3 className="chart-title">Último informe por banco</h3><DonutChart slices={bankSlices} /></div>
        <div className="chart-card"><h3 className="chart-title">Último informe por situación</h3><DonutChart slices={situacionSlices} /></div>
      </div>

      <div className="chart-card-wide"><h3 className="chart-title">Últimos 24 meses — deuda total</h3><BarChart data={barData} color="#F36B21" /></div>
      <div className="chart-card-wide"><h3 className="chart-title">Últimos 24 meses por entidad</h3><LineChart series={lineSeries} periods={periods.slice(-24)} /></div>

      <section style={{ marginBottom: '5mm' }}>
        <div className="section-title">Entidades con exposición histórica</div>
        <table className="data-table">
          <thead><tr><th>Entidad</th><th style={{ textAlign: 'right' }}>Monto total hist.</th><th style={{ textAlign: 'right' }}>Períodos</th><th style={{ textAlign: 'right' }}>Sit. máx.</th></tr></thead>
          <tbody>{entidadesHistoricas.map((row) => <tr key={row.entidad}><td>{row.entidad}</td><td style={{ textAlign: 'right' }}>{money(row.montoTotal)}</td><td style={{ textAlign: 'right' }}>{row.periodos}</td><td style={{ textAlign: 'right', color: situacionColor(row.maxSituacion), fontWeight: 850 }}>{row.maxSituacion || '—'}</td></tr>)}</tbody>
        </table>
      </section>
      ...
      <footer className="page-footer"><span>InmoRent · Informe Comercial Locativo</span><span>Información orientativa · {fmtDT(data.fetchedAt)}</span><span>2 / 2</span></footer>
    </section>
  );
}
