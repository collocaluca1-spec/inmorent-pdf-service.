...
const fmtPeriodo = (raw: string | null) => (!raw ? '—' : /^\d{6}$/.test(raw) ? `${raw.slice(4, 6)}/${raw.slice(0, 4)}` : raw);
...
function Field({ label, value }: { label: string; value: ReactNode }) {
  return <div><div className="field-label">{label}</div><div className="field-value">{value}</div></div>;
}

function SummaryCard({ label, value, accent }: { label: string; value: ReactNode; accent?: boolean }) {
  return <div className="summary-card"><div className="field-label">{label}</div><div className={`field-value ${accent ? 'value-brand' : ''}`}>{value}</div></div>;
}
    ...
    <section className="page">
      <header className="report-header">
        <div className="report-header-top">
          <div>
            <div className="brand-row">
              <div className="logo-box"><img src="/inmorent-logo.png" alt="InmoRent" /></div>
              <div>
                <div className="brand-title">InmoRent</div>
                <div className="brand-subtitle">Evaluación locativa</div>
              </div>
            </div>
            <div className="subject-eyebrow">Informe comercial locativo</div>
            <h1 className="subject-name">{data.denominacion || 'Sin denominación'}</h1>
            <div className="subject-meta">
              <span className="subject-doc-pill">{data.documento}</span>
              <span>{tipo}</span>
              <span>·</span>
        ...
        <div className="kpi-strip">
          <div className="kpi-cell"><div className="label">Deuda vigente</div><div className={`value ${valueRisk}`}>{money(data.deudaVigente)}</div><div className="value-small">período actual</div></div>
          <div className="kpi-cell"><div className="label">Situación vigente</div><div className={`value ${valueRisk}`}>{data.situacionVigente ? `Sit. ${data.situacionVigente}` : '—'}</div><div className="value-small">período actual</div></div>
          <div className="kpi-cell"><div className="label">Peor situación 24m</div><div className="value">{data.peorSituacion24m ? `Sit. ${data.peorSituacion24m}` : '—'}</div><div className="value-small">histórico</div></div>
          <div className="kpi-cell"><div className="label">Última consulta</div><div className="value">{fmtDate(data.fetchedAt)}</div><div className="value-small">{fmtDT(data.fetchedAt).split(',')[1]?.trim()}</div></div>
        </div>
      </header>
      ...
      <div className="main-grid">
        <div className="panel score-panel">
          ...
          <ScoreGauge score={data.score} color={data.color} size={130} />
        </div>
        ...
        <div className="panel info-panel">
          ...
          <div className="summary-grid">
            ...
            <SummaryCard label="Tendencia 6 meses" value={data.tendencia6m === 'estable' ? '✓ Estable' : data.tendencia6m === 'normalizada' ? '↗ Normalizada' : '↘ Inestable'} />
          </div>
        </div>
      </div>

      {data.situacionVigente === 1 && (data.peorSituacion24m ?? 0) >= 4 && (
        <div className="alert">Actualmente figura en situación normal, pero registra un antecedente histórico severo dentro de los últimos 24 meses. Se recomienda revisión manual antes de tomar una decisión.</div>
      )}

      <section className="table-section">
        <div className="section-title">Último informe bancario</div>
        <table className="data-table">
          <thead><tr><th>Entidad</th><th>Situación</th><th>Monto</th><th>Días atraso</th></tr></thead>
          <tbody>{data.actualRows.map((row, index) => <tr key={`${row.entidad}-${index}`}><td>{row.entidad}</td><td>{row.situacion}</td><td>{money(row.monto)}</td><td>{row.diasAtraso || '—'}</td></tr>)}</tbody>
        </table>
      </section>

      <footer className="page-footer"><span>InmoRent · Informe Comercial Locativo</span><span>Información orientativa · {fmtDT(data.fetchedAt)}</span><span>1 / 2</span></footer>
    </section>
  );
}
